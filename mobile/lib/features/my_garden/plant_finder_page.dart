...
class _PlantFinderPageState extends State<PlantFinderPage> {
  ...
  List<PlantSpeciesModel>? _results;
  bool _loading = false;
  ...
  Future<void> _loadQuestions() async {
    try {
      final qs = await _service.getFinderQuestions();
  ...
  Future<void> _choose(dynamic value) async {
    ...
    try {
      ...
      final results = await _service.getRecommendation(answers);
  ...
  Widget build(BuildContext context) {
    final progress = _questions.isEmpty ? 0.0 : ((_step + (_results != null ? 1 : 0)) / _questions.length) * 100;
        ...
        children: [
          ...
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              valueColor: const AlwaysStoppedAnimation(AppTheme.leaf600),
              minHeight: 6,
            ),
          ),
          ...
          Text(
            ...
            style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w800, color: AppTheme.leaf950),
          ),
      ...
      child: Column(
        children: [
          Container(
            width: double.infinity,
            padding: const EdgeInsets.all(20),
            decoration: BoxDecoration(color: AppTheme.leaf50, borderRadius: BorderRadius.circular(20)),
            child: Column(
              children: [
          ...
          ..._results!.asMap().entries.map((entry) {
            ...
            return Container(
              ...
              decoration: BoxDecoration(
                color: Colors.white,
                borderRadius: BorderRadius.circular(16),
                border: Border.all(color: i == 0 ? AppTheme.leaf400 : AppTheme.leaf100, width: i == 0 ? 2 : 1),
              ),
              child: Row(
                children: [
                  Text(_speciesEmoji(s.slug), style: const TextStyle(fontSize: 36)),
                  const SizedBox(width: 16),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
            ...
            children: [
              Expanded(
                child: OutlinedButton(
                  onPressed: () => setState(() { _step = 0; _answers = {}; _results = null; }),
  ...
  static const _defaultQuestions = [
    {
      ...
      'options': [
        {'value': 'indoor-terang', 'label': 'Dalam ruangan terang', 'icon': '🪟'},
        {'value': 'indoor-redup', 'label': 'Dalam ruangan redup', 'icon': '🛋️'},
        {'value': 'outdoor', 'label': 'Balkon / halaman', 'icon': '🌤️'},
        {'value': 'teras', 'label': 'Teras semi-terbuka', 'icon': '🏡'},
      ],
    },
    {
      'id': 'pengalaman', 'question': 'Seberapa berpengalaman Anda?', 'icon': '🧑‍🌾',
      'options': [
        {'value': 'pemula', 'label': 'Pemula', 'icon': '🌱'},
        {'value': 'sedang', 'label': 'Menengah', 'icon': '🌿'},
        {'value': 'mahir', 'label': 'Mahir', 'icon': '🪴'},
      ],
    },
    {
      'id': 'tujuan', 'question': 'Apa tujuan utama Anda?', 'icon': '🎯',
      'options': [
        {'value': 'hias', 'label': 'Mempercantik ruangan', 'icon': '🪴'},
        {'value': 'pangan', 'label': 'Panen sayur / buah', 'icon': '🥬'},
        {'value': 'udara', 'label': 'Membersihkan udara', 'icon': '💨'},
        {'value': 'relaksasi', 'label': 'Hobi & relaksasi', 'icon': '🧘'},
      ],
    },
    {
      'id': 'waktu', 'question': 'Waktu merawat per minggu?', 'icon': '⏰',
      'options': [
        {'value': 'sibuk', 'label': 'Sangat sibuk (<15 menit)', 'icon': '⚡'},
        {'value': 'normal', 'label': 'Cukup (15–45 menit)', 'icon': '☀️'},
